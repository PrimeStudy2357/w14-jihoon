...
router.post("/", function (req, res, next) {
  ...
  // JSON 형태로 전달되었음을 가정하고 그 안에 있는 message 값을 읽어온다.
  const userMessage = req.body.message;

  // 사용자로부터 받은 정보를 서비스에 사용.
  // 여기선 메시지를 서버측 파일에 기록하는 서비스.
  fs.appendFileSync("db.txt", userMessage);

  // 처리가 잘 되었음을 알려주는 메시지를 응답 객체에 담아서 전달함
  res.status(201).json({
    message: "DONE",
  });
});
...
router.delete("/", function (req, res, next) {
  // 해당 경로의 기능은 현재 제공되지 않음을 나타내는 메세지를 반환하기

  // 이 경우 평문(Plain Text)를 응답으로 보내고 있음
  res.status(500).send("Not implemented !!!");
});

/** http://localhost:3000/users 경로에 특정 가변값(:userId)이 포함된 요청이 들어올 경우 처리 */
router.get("/users/:userId", function (req, res, next) {
  // userId 값을 가지는 사용자에 대한 정보를 반환하기
